        ...
        function Element() {
            this.initialize = function() {
                this.elementType = "element",
                    this.serializedProperties = ["elementType", "id"],
                      ...
                      if(typeof f == "object"){
                        ...
                        if( f instanceof  Array){
                          if(f.length==0){
                            ...
                            data='[';
                            for(var ind1=0;ind1< f.length;ind1++){
                              data+="{"+'"'+"value"+'"'+':"' + f[ind1].value + '",';
                              data+='"'+"operaterType"+'"'+':"'+ f[ind1].operaterType +  '",';
                              data+='"'+"property"+'"'+':"' + f[ind1].property + '"},';
                            }
                          }
                          ...
                          data='{';
                          for (var k in f) {
                            data += '"'+k+'"' + ":{"+'"'+"property"+'"'+':"' + f[k].property + '"';
                            if (typeof (f[k]).value == 'object') {
                              var data1 = [];
                              for (var z in f[k].value) {
                                if (typeof(f[k].value[z]) == 'object') {
                                  data1.push('{"name":' + '"' + f[k].value[z].name + '","value":' + '"' + f[k].value[z].value +
                                    '","value1":' + '"' + f[k].value[z].value1 + '","value2":' + '"' + f[k].value[z].value2 + '"}');
                                } else {
                                  break
                                }
                              }
                ...
                createStageFromJson: function(jsonObj, canvas) {
                    ...
                    return scenes.forEach(function(a) {
                            ...
                            d.forEach(function(a) {
                                    ...
                                    if ("node" == d) {
                                        c = new JTopo.Node;
                                      //c.deviceId = "" + new Date().getTime() * Math.random();
                                      //c.props={};
                                      //c.condition=[];
                                      //
                                      ////每个节点创建单独的属性对象
                                      //getSideBarObj(modalData,attribute,c);
                                        c.alpha = editor.config.alpha;
                                        c.strokeColor = editor.config.nodeStrokeColor;
                                        c.fillColor = editor.config.fillColor;
                                        c.shadow = editor.config.shadow;
        ...
        function Stage(c) {
            ...
            this.initialize = function(c) {
                initJtopoEvent(c),
                    ...
                    this.mouseOver = !1,
                    this.needRepaint = !0,
                    this.serializedProperties = ["wheelZoom", "width", "height", "id"],
                    this.nextLevel = "0"
            }
                ,
        ...
        function b(c) {
            this.initialize = function(c) {
                b.prototype.initialize.apply(this, arguments),
                    ...
                    this.inLinks = [],
                    this.outLinks = [];
                var d = "bussClass,time,deviceId,dataType,nodeImage,text,textPosition,props".split(",");
            ...
            this.initialize = function(b, c, d) {
                if (f.prototype.initialize.apply(this, arguments),
                    ...
                    null  != this.nodeZ && this.nodeZ.inLinks.push(this),
                        ...
                        this.path = [];
                    var e = "deviceA,deviceZ,lineType,text,condition".split(",");
